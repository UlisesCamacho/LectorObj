...
GLfloat B = 0.0f;
GLfloat A = 0.0f;
float LightPos2[] = { 1.0f, 0.5f, 1.0f, 0.0f };   // Light Position
float LightAmb2[] = { 0.2f, 0.2f, 0.2f, 1.0f };   // Ambient Light Values
float LightDif2[] = { 1.0f, 1.0f, 1.0f, 1.0f };   // Diffuse Light Values
float LightSpc2[] = { 1.0f, 1.0f, 1.0f, 1.0f };   // Specular Light Values
...
void Objeto::draw()
{
	...
	/* Activo la fuente de luz */
	glLightfv(GL_LIGHT1, GL_POSITION, LightPos2);        // Set Light1 Position
	glLightfv(GL_LIGHT1, GL_AMBIENT, LightAmb2);         // Set Light1 Ambience
	glLightfv(GL_LIGHT1, GL_DIFFUSE, LightDif2);         // Set Light1 Diffuse
	glLightfv(GL_LIGHT1, GL_SPECULAR, LightSpc2);        // Set Light1 Specular
	...
	setPixel(anguloCuboX, anguloCuboY);
				...
				if (cambio == 1) //empieza animacion
				{
					...
					for (int i = 1; i <= 3; i++)
					{
						if (numero == 0)
						{
							...
							case 1:
								cout << "Encesta\n";
								abc[0].setxyz(-0.9, 0.035, 0.1);
								abc[1].setxyz(-0.1, 1.0, 0.1);
								abc[2].setxyz(0.0, 1.0, 0.1);
								abc[3].setxyz(0.1, 0.035, 0.1);
								nPuntos = 4;
								break;
							case 2:
								abc[0].setxyz(0.1, 0.035, 0.1);
								abc[1].setxyz(0.2, 0.5, 0.1);
								abc[2].setxyz(0.3, 0.5, 0.1);
								abc[3].setxyz(0.4, 0.035, 0.1);
								nPuntos = 4;
								break;
							case 3:
								abc[0].setxyz(0.4, 0.035, 0.1);
								abc[1].setxyz(0.5, 0.3, 0.1);
								abc[2].setxyz(0.6, 0.3, 0.1);
								abc[3].setxyz(0.7, 0.035, 0.1);
								nPuntos = 4;
								break;
							default:
								break;
						...
						else
						{
							...
							case 2:
								...
								abc[3].setxyz(-0.3, 0.035, 0.1);
								nPuntos = 4;
								break;
							case 3:
								abc[0].setxyz(-0.3, 0.035, 0.1);
						...
						for (double t = 0.0; t <= 1.0; t += 0.009) {
							...
							for (int i = 0; i < o.indicesface.size(); i++)
							{
								...
								glVertex3f(vPuntos2[0], vPuntos2[1], vPuntos2[2]);
								glEnd();
								glDisable(GL_LIGHT0);                                // Enable Light1
								glDisable(GL_LIGHTING);
